...
_DETAILS = {
    ...
    "mission": (
        ...
        "/mission scan                  Missions des screenshots récents (défaut : 2h)\n"
        ...
        "Alias : /m\n\n"
        "Workflow scan (overlay) :\n"
        "  1. Dans SC : ouvrir Contrats → prendre des screenshots (F12)\n"
        "  2. Le badge 📷 s'allume dans la barre de statut → cliquer\n"
        "     (ou taper /mission scan)\n"
        "  3. Sélectionner les missions souhaitées\n"
        "  4. Bouton 'Ajouter au catalogue' ou '+ Voyage actif'\n\n"
        "Catégories détectées automatiquement depuis le titre et les objectifs :\n"
        "  Hauling stellaire · Hauling interstellaire · Récupération\n"
        "  Chasse aux primes · Mercenaire · Collecte · Enquête\n"
        "  Livraison · Minage · PvP\n\n"
        "/help mission scan   → détail du scan par screenshot\n"
    ...
    "mission scan": (
        "/mission scan [scope]\n\n"
        "  Affiche les missions extraites des screenshots par l'OCR background.\n"
        "  Les screenshots sont analysés automatiquement dès détection (auto_ocr).\n"
        "  La base est stockée dans ~/.uexinfo/screenshot_db.json\n\n"
        "  Scopes disponibles :\n"
        "    (rien)    Fenêtre configurable (défaut : 2h) → /config scan hour <n>\n"
        "    all       Toute la base, toutes dates\n"
        "    today     Captures depuis minuit\n"
        "    terminal  Terminaux scannés (pas les missions)\n\n"
        "  En overlay : panneau avec cases à cocher par session, puis :\n"
        "    'Ajouter au catalogue'  → crée les missions dans /mission list\n"
        "    '+ Voyage actif'        → crée + ajoute au voyage actif\n\n"
        "  En CLI pur : tableau numéroté, saisie sélection (ex: 1,3,5-8 ou all)\n"
        "  puis choix catalogue (c) ou voyage actif (v).\n\n"
        "  Sessions : un gap > session_gap minutes entre deux screenshots\n"
        "  = nouvelle session. Configurable : /config scan session_gap <min>\n\n"
        "  La clé de déduplication est le fichier source — chaque screenshot\n"
        "  est une entrée unique. Plusieurs captures de la même mission\n"
        "  (animation SC) apparaissent séparément ; sélectionner la dernière.\n\n"
        "  Exemples :\n"
